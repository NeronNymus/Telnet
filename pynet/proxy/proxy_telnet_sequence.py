        ...
        try:
            ...
            if len(client_buffer):
                ...
                if remote_buffer != b'':
                    ...
                    # We are going to append the remote_buffer instead into a file
                    outpath = "/home/grimaldi/Bash/Telnet/pynet/proxy/help_messages.txt"

                    with open(outpath, 'a') as outfile:

                        # Decoding optional
                        remote_buffer = remote_buffer.decode('ascii')

                        outfile.write(remote_buffer)
                        print("[!!] Decodeded:\n%s" % (remote_buffer))
                        print("[!!] Result saved into: [%s]\n" % (os.path.basename(outpath)))
