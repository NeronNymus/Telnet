...
def fuzz_proxy():
    ...
    interfaces_path = "/mnt/Kali/home/grimaldi/Bash/Telnet/analysis/wap_interfaces"
    ...
    with open(interfaces_path, 'r') as file:
        for interface in file:
            ...
            command = f"display dns proxy info interface {interface}\r\n".encode()
...
# Find unique MAC's from a log file
def find_mac_addresses(file_path):
    ...
    unique_sorted_mac_addresses = sorted(set(mac_addresses))
    
    # Join tuples into full MAC address stings
    return unique_sorted_mac_addresses


# This method tries to enable ssh by fuzzing it
def enable_ssh(private_ip, file_path):
    mac_addresses = find_mac_addresses(file_path)

    commands_seq = list()

    print(f"\n[!] Found MAC addresses of the device {private_ip}\n")

    output_path = f"logs2/{private_ip}_macs"

    with open(output_path, 'w') as file:
        for mac_address in mac_addresses:
            mac_address = mac_address.lower()
            command = f"set apssh mac {mac_address} enable 1\r\n".encode()
            commands_seq.append(command)

            file.write(mac_address + "\n")

        print(f"[!] MAC's written to:\n{output_path}\n")
...
if __name__ == "__main__":
    ...
    fuzz_list = fuzz_proxy()

    #private_ip = "10.234.7.203"
    #file_path = f"logs2/{private_ip}_log2"
    #fuzz_list = enable_ssh(private_ip, file_path)
